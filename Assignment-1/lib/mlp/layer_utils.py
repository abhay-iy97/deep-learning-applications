...
class fc(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        W1 = self.params[self.w_name]
        b1 = self.params[self.b_name]
        b1 = b1.reshape(b1.shape[0], 1)
        Z1 = (np.dot(feat, W1)).T + b1
        output = Z1.T
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
class leaky_relu(object):
    ...
    def forward(self, feat):
        ...
        # Store the results in the variable output provided above.                  #
        #############################################################################
        temp = np.where(feat >= 0, 1, self.negative_slope)
        output = np.multiply(temp,feat)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        loss = - (1 / number_of_examples) * np.sum(np.log(logit[range(number_of_examples),label]))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
    ...
    def backward(self):
        ...
        number_of_examples = label.shape[0]
        dlogit = logit
        dlogit[range(number_of_examples), label] -= 1
        dlogit /= number_of_examples
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
